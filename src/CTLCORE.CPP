...
BOOL COleControl::OnProperties(LPMSG, HWND hWndParent, LPCRECT)
{
		...
		FAILED(hr = m_pControlSite->ShowPropertyFrame()))
	{
		...
		hr = _AfxOleCreatePropertyFrame(hWndOwner, rectParent.left - rectTop.left,
...
void COleControl::OnPaint(CDC* pDC)
{
	...
	{
// REVIEW: BeginPaint in WLM clears an internal flag that tells Windows to
// send a WM_PAINT.  It also sends WM_NCPAINT with the correct region, and
// removes the frame region from the window's real update region.  The update
// region in a PAINTSTRUCT is only  for the client area, so validating it
// doesn't validate the nonclient area.
// Finally, the function that sends the WM_PAINT checks if the internal
// "needs WM_PAINT" flag has been cleared, and if not, emits a nasty debug
// message, and sends an NCPAINT anyway.  I'm willing to bet Intel does the
// same thing, only nobody ever noticed because no one runs debug Windows
// anymore.  This code works just fine, I just ifdef'd it out to
// eliminate the RIPs.
...
COLORREF COleControl::TranslateColor(OLE_COLOR clrColor, HPALETTE hpal)
{
	...
	_AfxOleTranslateColor(clrColor, hpal, &cr);
	return cr;
}
