...
void CFontPropPage::FillFacenameList()
{
	...
	CClientDC dc(NULL);
#ifndef _MAC
	EnumFontFamilies(dc.GetSafeHdc(), (LPCTSTR) NULL, (FONTENUMPROC) CFontPropPage::EnumFontFamiliesCallBack, (LPARAM) this);
#else
	ENUMLOGFONTMINIMAL elfm;
	UINT iFont = 0;
	UINT nFonts;

	memset(&elfm.lf, 0, sizeof(LOGFONT));
	while (EnumFontsMinimal(NULL, &elfm, 1, &nFonts, &iFont))
	{
		UINT i = 0;
		VERIFY(EnumFontsMinimal(elfm.lf.lfFaceName, &elfm, 1, &nFonts, &i));
		m_FontNames.AddFont(&elfm.lf, elfm.dwType);
	}
