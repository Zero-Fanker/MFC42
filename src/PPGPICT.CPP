...
void CPicturePropPage::OnBrowse()
{
	...
	LPPICTURE pPict;
	if (SUCCEEDED(_AfxOleLoadPicture(pstm, lSize, FALSE, IID_IPicture, (LPVOID *)&pPict)))
	{
		ChangePicture(pPict);
	}
	else
...
static PICTDESC _pictDescEmpty =
{
	sizeof(PICTDESC),
	PICTYPE_NONE,
};
...
{
	LPPICTURE pPict;
	if (SUCCEEDED(_AfxOleCreatePictureIndirect(&_pictDescEmpty, IID_IPicture, TRUE,
		(LPVOID *)&pPict)))
	{
		ChangePicture(pPict);
	}
