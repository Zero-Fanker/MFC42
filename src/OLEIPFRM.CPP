...
COleCntrFrameWnd::COleCntrFrameWnd(COleIPFrameWnd* pInPlaceFrame)
{
	...
	if (NULL == rghCursors[1])
	{
		HINSTANCE hInst = AfxFindResourceHandle(
			MAKEINTRESOURCE(AFX_IDC_TRACK4WAY), RT_GROUP_CURSOR);
		rghCursors[0] = ::LoadCursor(hInst, MAKEINTRESOURCE(AFX_IDC_MOVE4WAY));
		rghCursors[1] = ::LoadCursor(hInst, MAKEINTRESOURCE(AFX_IDC_TRACKNWSE));
		rghCursors[2] = rghCursors[1];
		rghCursors[3] = ::LoadCursor(hInst, MAKEINTRESOURCE(AFX_IDC_TRACKNS));
		rghCursors[4] = ::LoadCursor(hInst, MAKEINTRESOURCE(AFX_IDC_TRACKNESW));
		rghCursors[5] = ::LoadCursor(hInst, MAKEINTRESOURCE(AFX_IDC_TRACKWE));
		rghCursors[6] = rghCursors[5];
		rghCursors[7] = rghCursors[4];
		rghCursors[8] = rghCursors[3];
		rghCursors[9] = rghCursors[2];
	}
#endif
}
...
#ifdef _MAC
// Mac OLE in-place hatch border support
BOOL COleIPFrameWnd::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	if (GetProp(pWnd->m_hWnd, "Owner") == m_hWnd)
	{
		POINT point;
		int hit;
		::GetCursorPos(&point);
		hit = WlmOleWhichGrowHandle(m_hWnd, &point) + 1;
		if (hit >= 0 && hit <= 9)
		{
			::SetCursor(rghCursors[hit]);
			return TRUE;
		}
	}
...
#else
	// Macintosh OLE automatically clips the object
	AfxRepositionWindow(NULL, m_hWnd, &rect);
#endif
