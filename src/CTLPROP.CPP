...
void COleControl::SetBackColor(OLE_COLOR clrBackColor)
{
	...
	if (FAILED(_AfxOleTranslateColor(clrBackColor, NULL, NULL)))
		ThrowError(CTL_E_INVALIDPROPERTYVALUE, AFX_IDP_E_INVALIDPROPERTYVALUE);
...
void COleControl::SetForeColor(OLE_COLOR clrForeColor)
{
	...
	if (FAILED(_AfxOleTranslateColor(clrForeColor, NULL, NULL)))
		ThrowError(CTL_E_INVALIDPROPERTYVALUE, AFX_IDP_E_INVALIDPROPERTYVALUE);
...
static inline
DWORD AfxTransferFileContent(CFile* pFrom, CFile* pTo)
{
	BYTE buff[1024];
	DWORD dwRead = 0;
	DWORD dwActual;
...
void CCachedDataPathProperty::OnDataAvailable(DWORD dwSize, DWORD bscfFlag)
{
	...
	TRY
	{
		...
		m_Cache.SeekToEnd();
		AfxTransferFileContent(this, &m_Cache);
	}
	CATCH_ALL(e)
	{
