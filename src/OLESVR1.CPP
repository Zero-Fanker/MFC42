...
#else
		{
			// If a Mac user quits the app while IP sessions are active,
			// it must shut them down and exit, since we can't remove
...
#ifndef _MAC
	// send activate notification.  This really is the right time to do it,
	// but Mac Word 6 requires later notification
...
#ifdef _MAC
	// Mac Word 6 also requires this reordering.  Hopefully future versions
	// will be able to use the standard Win32 activation sequence
	if (lpInPlaceSite->OnUIActivate() != S_OK)
		goto DeactivateUIAndFail;
...
#else
		// Some Mac containers call OnDeactivate within their RemoveMenus
		// methods.  Prevent recursion in this case.
		if (pThis->m_pInPlaceFrame != NULL && !pThis->m_bDeactivating)
		{
			pThis->OnDeactivate();
